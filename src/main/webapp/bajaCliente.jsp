...
%>
<head>
    <title>ZgameK - Baja Cliente</title>
    <meta charset="utf-8">
...
<body>
    ...
    <div class="container d-flex align-items-center justify-content-center" style="min-height: 60vh;">
        <div class="col-12 col-md-8 col-lg-6">
            <h1 class="text-center">Baja Cliente</h1>
            <p class="text-center">Elige el cliente que quieres dar de baja</p>
            <form action="operaciones.jsp?tipo=bajaCliente" method="post" accept-charset="UTF-8" style="display: flex; flex-direction: column; gap: 2rem">
                <div class="mb-3" style="margin: 3rem;">
                    <select class="form-select" id="cliente" name="cliente">
                    	<%boolean primero = true;
                    	  for(Cliente c : clientes){ %>
	                    	<%if (primero) { %>
	                        	<option value="<%=c.getId()%>"selected style="font-size: 20px"><%=c.getApellidos()%>, <%=c.getNombre()%> - <%=c.getDni()%></option>
	                        <%primero=false;
	                          }else{ %>
	                        	<option value="<%=c.getId()%>" style="font-size: 20px"><%=c.getApellidos()%>, <%=c.getNombre()%> - <%=c.getDni()%></option>
	                        <%}
                          } %>
                    </select>
                </div>
