...
ArrayList<LineaVenta> lVentas = cbd.todasLineasVentaDeVenta(idVenta);


%>
<head>
    <title>ZgameK - Linea Ventas</title>
    <meta charset="utf-8">
    ...
    <style>
        ...
        .customer-listing .customer-table-container {
            width: 80%;
        ...
        .customer-listing tbody td:nth-child(4) {
            cursor: pointer;
            color: #3030ff;
            transition: color 0.3s;
        }
        .customer-listing tbody td:nth-child(5) {
            cursor: pointer;
            color: #3030ff;
            transition: color 0.3s;
        }
        
        .customer-listing tbody tr {
            cursor: pointer;
            transition: color 0.3s;
        }

        .customer-listing tbody td:nth-child(4):hover {
            color: #AB7B59;
        }
        .customer-listing tbody td:nth-child(5):hover {
            color: #AB7B59;
        }
        
        

        .customer-listing tbody tr td {
            padding: 10px;
            vertical-align: middle;
            text-align: center;
        }

        .customer-listing img {
            margin-right: 10px;
            border-radius: 50%;
            width: 50px;
            height: 50px;
...
<body>
    ...
    <!-- Close Header -->
    <div class="customer-listing" style="margin-bottom:3rem; margin-top:3rem;">
        <h2 class="text-center" style="margin-bottom:3rem;">Línea de venta</h2>
        <div class="customer-table-container">
            <table class="table table-striped">
                ...
                <tbody>
                    <%for(LineaVenta lV: lVentas) {%>
                    <%Juego j = cbd.dameJuego(lV.getIdJuego()); %>
                    <tr>
                        <td><%=lV.getIdLVenta() %></td>
                        <td><%=lV.getIdVenta() %></td>
                        <td><%=lV.getIdJuego() %></td>
                        <td onclick="window.location.href='producto.jsp?idJuego=<%=j.getId()%>'"><b><%=j.getNombre() %></b></td>
                        <td onclick="window.location.href='shop.jsp?plataforma=<%=j.getPlataforma()%>'"><b><%=j.getPlataforma() %></b></td>
                        <td><%=j.getPrecio()*lV.getUnidades() %> €</td>
                        <td><%=lV.getUnidades() %></td>
                    </tr>
                    <%}%>
                </tbody>
            </table>
