...
juegoAux = cbd.dameJuego(idJuego);


%>

<head>
    <title>ZgameK - Modificar Juego</title>
    <meta charset="UTF-8">
...
<body>
    ...
    <!-- Close Header -->
    <!-- Formulario de Modificacion de Juego -->
    <div class="container d-flex align-items-center justify-content-center" style="min-height: 80vh;">
        <div class="col-12 col-md-8 col-lg-6">
            <h1 class="text-center">Modificar Juego</h1>
            <form action="operaciones.jsp?tipo=modificarJuego&idJuego=<%=juegoAux.getId()%>&imagenes=<%=juegoAux.getNumImagenes()%>" method="post" accept-charset="UTF-8">
                <div class="mb-3">
                    <label for="nombre" class="form-label">Nombre del juego</label>
                    <input type="text" class="form-control" id="nombre" name="nombre" value="<%=juegoAux.getNombre()%>" maxlength="26" required>
                </div>
                <div class="mb-3">
                    <label for="plataforma" class="form-label">Plataforma</label>
                    <select class="form-select" id="plataforma" name="plataforma" required>
                        <%for(String p: cbd.todasPlataformas()) { 
                            if (p.equalsIgnoreCase(juegoAux.getPlataforma())) {%>
                                <option value="<%=juegoAux.getPlataforma()%>" selected><%=juegoAux.getPlataforma()%></option>
                            <%}else{%>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="categoria" name="categoria" required>
                        <%for(String p: cbd.todasCategorias()) { 
                            if (p.equalsIgnoreCase(juegoAux.getCategoria())) {%>
                                <option value="<%=juegoAux.getCategoria()%>" selected><%=juegoAux.getCategoria()%></option>
                            <%}else{%>
                                <option value="<%=p%>"><%=p%></option>
                            <%}
                          }%>
                ...
                <div class="mb-3">
				    ...
				    <textarea class="form-control" rows="5" id="descripcion" name="descripcion" maxlength="200" required><%=juegoAux.getDescripcionCorta()%></textarea>
				</div>
                <div class="mb-3">
                    <label for="precio" class="form-label">Precio €</label>
                    <input type="number" class="form-control" id="precio" name="precio" min="1" max="70" step="0.01" value="<%=juegoAux.getPrecio()%>" required>
                </div>
                <div class="mb-3">
                    <label for="valoracion" class="form-label">Valoración</label>
                    <input type="number" class="form-control" id="valoracion" name="valoracion" min="0" max="5" value="<%=juegoAux.getValoracion()%>" required>
                </div>
    ...
    <!-- End Formulario de Modificacion de Juego -->
<!-- Start Footer -->
